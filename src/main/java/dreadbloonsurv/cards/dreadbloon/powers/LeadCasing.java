...
public class LeadCasing extends AbstractEasyCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m)
    {
        ...
        for (AbstractPower pp : m.powers) {
            ...
            double predictedThreat = (((bb.amount2 ) * mult +
                    bb.amountShield) / Math.max(0.5, delay - (double)(bb.amountArmor + magicNumber) / 2)) - (((bb.amount2) * mult +
                    bb.amountShield) / Math.max(0.5, delay - (double) bb.amountArmor / 2));

            double currentBestThreat = ((best == null) ? -1 :
                    (((best.amount2) * bestMult +
                            best.amountShield) / Math.max(0.5, bestDelay - (double)(best.amountArmor + magicNumber) / 2)) - (((best.amount2) * bestMult +
