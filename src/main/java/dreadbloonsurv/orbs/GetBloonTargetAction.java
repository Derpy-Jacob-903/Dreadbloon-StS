...
public class GetBloonTargetAction extends AbstractGameAction {
    public boolean hasStarted;
    private AbstractMonster hovered;
    private java.util.function.Consumer<AbstractBloon> onChosen;
    private AbstractCreature source;
    private static final int SEGMENTS = 20;
    private Vector2[] points = new Vector2[20];
    private Vector2 controlPoint;
    private float arrowScaleTimer = 0.0F;
    private AbstractBloon hoveredMonster = null;

    public GetBloonTargetAction(AbstractCreature source, java.util.function.Consumer<AbstractBloon> onChosen) {
        this.source = source;
    ...
    private void renderTargetingUi(SpriteBatch sb) {
        ...
        this.controlPoint = new Vector2(this.source.hb_x - (x - this.source.hb_y) / 4.0F, y + (y - this.source.hb_y - 40.0F * Settings.scale) / 2.0F);
        float arrowScale;
        ...
        tmp.nor();
        this.drawCurvedLine(sb, new Vector2(this.source.hb_x, this.source.hb_y - 40.0F * Settings.scale), new Vector2(x, y), this.controlPoint);
