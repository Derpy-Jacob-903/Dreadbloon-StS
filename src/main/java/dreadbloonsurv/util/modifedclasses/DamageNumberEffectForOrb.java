...
public class DamageNumberEffectForOrb  extends AbstractGameEffect {
    ...
    public AbstractOrb target;

    public DamageNumberEffectForOrb(AbstractOrb target, float x, float y, int amt) {
        this.duration = 1.2F;
        ...
        if (Settings.SHOW_DMG_SUM && amt > 0) {
            ...
            for(AbstractGameEffect e : AbstractDungeon.effectList) {
                if (e instanceof SumDamageEffectForOrb && ((SumDamageEffectForOrb)e).target == target) {
                    isSumDamageAvailable = true;
                    ((SumDamageEffectForOrb)e).refresh(amt);
                }
            }

            if (!isSumDamageAvailable) {
                for(AbstractGameEffect e : AbstractDungeon.effectList) {
                    if (e instanceof DamageNumberEffectForOrb && e != this && ((DamageNumberEffectForOrb)e).target == target) {
                        AbstractDungeon.effectsQueue.add(new SumDamageEffectForOrb(target, x, y, ((DamageNumberEffectForOrb)e).amt + amt));
                    }
                }
            }
